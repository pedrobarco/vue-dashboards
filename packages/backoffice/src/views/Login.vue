        ...
        <div class="flex sm:bg-white sm:shadow-md sm:rounded-lg">
          <div class="hidden w-1/2 rounded-l lg:block">
            <div class="flex items-center h-full">
              <img class="ml-5 fill-current" src="/img/login-bg.svg" alt="Dashboard" />
            </div>
          </div>
          <div class="w-full px-8 rounded-r relative sm:px-12 sm:py-24 lg:w-1/2 lg:px-24 lg:py-32">
            <img
              class="hidden absolute h-12 top-0 right-0 mt-8 mr-8 sm:block"
              src="/img/logo-mini.svg"
              alt="Logo"
            />
            <h3 class="text-gray-800 text-4xl font-bold">{{ $t("views.Login.title") }}</h3>
            <p class="w-2/3 mt-1 text-gray-500">{{ $t("views.Login.description") }}</p>
            <form @submit.prevent="submitLoginForm" class="mt-8">
              ...
              <div :class="error ? 'mt-5' : 'mt-10'">
                <button
                  ...
                  type="submit"
                  data-cy="submit"
                >{{ $t("views.Login.form.submit") }}</button>
              </div>
            </form>
          </div>
        </div>
